...
namespace UFE3D
{
  public class HitBoxesScript : MonoBehaviour
  {
    ...
    private FPVector worldPosition
    { get 
      { 
        return controlsScript != null ? controlsScript.worldTransform.position : FPVector.ToFPVector(transform.position); 
      } 
      set 
      { 
        controlsScript.worldTransform.position = value; 
      } 
    }
    ...
    void OnDrawGizmos()
    {
      ...
      foreach (HitBox hitBox in hitBoxes)
      {
        ...
        if (UFE.GetActiveConfig() == null || !UFE.GetActiveConfig().detect3D_Hits)
        {
          // Removing for brawler, keep hitboxes where they are in z
          //hitBoxPosition.z = -1;
        }
      ...
      if (blockableArea != null && blockableArea.enabled && previewAllBoxes)
      {
        ...
        if (UFE.GetActiveConfig() == null || !UFE.GetActiveConfig().detect3D_Hits)
        {
          // remove for brawler, keep block areas where they are in z (not that we'll need blocking)
